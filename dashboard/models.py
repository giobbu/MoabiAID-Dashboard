...
class Truck(models.Model):
    ...
    # TODO: check if we need to handle possible 6d and RDE values
    euro_value = models.PositiveSmallIntegerField(null=True)

    # Geographic information
    # TODO: Update to MobilityDB when/if we deploy to this (Django custom Field subclass)
    ...
    class Meta:
        ...
        indexes = [
            psql_indexes.BrinIndex(
                fields=['measurement_date'], name='daily_summary_idx')
        ]


class TrafficStateMixin:
    def update_typical(self, daily_state, day_of_week):
        """
        Update the values of typical traffic state for this commune on the given day of the week.
        State includes the number of trucks and thier average speed for every hour of the day.
        This should be called on every daily reset of the traffic state.

        The typical metrics are as follows: 
        - Mean: the mean value over the last five years (i.e. 52 weeks * 5 years = 260 = n) NOTE: this may change if a more appropriate scheme is found
        - Maximum: Historical maximum over ALL data that was ever recieved for this value
        - Minumim: Historical minimum over all data

        :param daily_state: The current days state dict that was generated by the real-time module 
        :type daily_state: dict
        :param day_of_week: Current day of the week (0 = Monday, 1 = Tueseday, ...)
        :type day_of_week: int
        """
        typical_states = self.typical_traffic[day_of_week]

        daily_state = {}
        for i in range(24):
            # TODO: compute the new values and assing to dail_state dict
            pass

        self.typical_traffic = typical_states.update(daily_state)
...
class Commune(models.Model):
    ...
    # Data attributes
    # One flexible JSON field to handle multiple state formats
    typical_traffic = JSONField(default=dict)
...
class Street(models.Model):
    ...
    ONE_WAY_CHOICES = [('F', 'Line direction'),
                       ('T', 'Oposite diretion'), ('B', 'Two Way')]
    one_way = models.CharField(
        max_length=1, default='B', choices=ONE_WAY_CHOICES)
    bridge = models.BooleanField(default=False)
    tunnel = models.BooleanField(default=False)

    commune = models.ForeignKey("dashboard.Commune", on_delete=models.DO_NOTHING,
                                related_name='streets')  # Every street belongs to a commune

    # TODO: review, either change pipeline code or store streets as ploygons
